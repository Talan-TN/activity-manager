...
import org.activiti.rest.mock.ActivitiDelegateMocker;
...
public class TaskResource extends TaskBaseResource {
  ...
  @Autowired
  private ActivitiDelegateMocker activitiDelegateMocker;

  @Value("${activiti.delegate.mock.enabled:false}")
  private Boolean delegateMockEnabled = false;
  ...
  protected void completeTask(Task task, TaskActionRequest actionRequest) {
    ...
    if(this.delegateMockEnabled){
      this.activitiDelegateMocker.mockProcessDefinitionServiceTasksAndListeners(task.getProcessDefinitionId());
    }
