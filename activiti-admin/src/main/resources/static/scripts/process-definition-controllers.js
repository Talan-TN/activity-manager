...
activitiAdminApp.controller("ProcessDefinitionController", [
  "$scope",
  "$rootScope",
  "$http",
  "$timeout",
  ...
  function(
    ...
    $scope.filterConfigLoaded = false;
    $scope.columnsNamesAndType = [];
    $scope.columns = [];
    $scope.columnsDataTypes = [];
    $scope.filterInputs = [];
    $scope.filterData = [];
    $scope.processIds = [];
    $scope.processIdColumnName = "";
    $scope.prcoessInstancesFound = true;

    $scope.addFilterInput = function(item) {
      if ($scope.columns.indexOf(item.key) == -1) {
        $scope.columns.push(item.key);
        $scope.columnsDataTypes.push(item.value);
        $scope.filterInputs.push("");
      }
    };

    $scope.removeInput = function(index) {
      $scope.filterInputs.splice(index, 1);
      $scope.columns.splice(index, 1);
    };

    $scope.search = function() {
      $scope.filterInputs.forEach((val, index) => {
        $scope.filterData.push({
          key: $scope.columns[index],
          value: val,
          type: $scope.columnsDataTypes[index]
        });
      });

      if ($scope.filterConfigLoaded) {
        $scope.filtersConfigData.forEach(data => {
          if (data.processDefinitionKey == $scope.definition.key)
            $scope.processIdColumnName = data.processIdColumnName;
        });
      }
      ...
      $http({
        ...
        .success(function(data, status, headers, config) {
          ...
          $scope.tabData.tabs[0].info = $scope.processIds.length;
          $scope.processIds.forEach(pid => {
            $http({
              method: "GET",
              url: "/rest/activiti/process-instances/" + pid
            })
              .success(function(data, status, headers, config) {
                $scope.processInstances.data.push(data);
              })
              .error(function(data, status, headers, config) {
                $scope.prcoessInstancesFound = false;
              });
          });
          if ($scope.prcoessInstancesFound == false) {
            $scope.addAlert("some processes Instances not found", "error");
          }
          $scope.prcoessInstancesFound = true;
          $scope.filterData = [];
        })
        .error(function(data, status, headers, config) {
          $scope.addAlert("Search error", "error");
          $scope.filterData = [];
        });
    };
    ...
    $scope.tabData = {
      tabs: [
        {
          id: "processInstances",
          name: "PROCESS-DEFINITION.TITLE.PROCESS-INSTANCES"
        },
    ...
    $scope.editCategory = function() {
      ...
      modalInstance.result.then(function(data) {
        if (data) {
          $scope.addAlert(
            $translate.instant(
              "ALERT.PROCESS-DEFINITION.CATEGORY-UPDATED",
              $scope.definition
            ),
            "info"
          );
    ...
    $scope.loadProcessInstances = function() {
      ...
      $http({
        method: "GET",
        url:
          "/rest/activiti/process-definitions/" +
          $scope.definition.id +
          "/process-instances"
      }).success(function(data, status, headers, config) {
        $scope.processInstances = data;
    ...
    $scope.loadJobs = function() {
      ...
      $http({
        method: "GET",
        url:
          "/rest/activiti/process-definitions/" + $scope.definition.id + "/jobs"
      }).success(function(data, status, headers, config) {
        $scope.jobs = data;
    ...
    function loadStartForm() {
      ...
      $http({
        method: "GET",
        url:
          "/rest/activiti/process-definition-start-form/" + $scope.definition.id
      }).success(function(data, status, headers, config) {
    ...
    $scope.loadDecisionTables = function() {
      ...
      $http({
        method: "GET",
        url:
          "/rest/activiti/process-definition-decision-tables/" +
          $scope.definition.id
      })
        .success(function(data, status, headers, config) {
    ...
    $scope.loadForms = function() {
      ...
      $http({
        method: "GET",
        url: "/rest/activiti/process-definition-forms/" + $scope.definition.id
      })
        .success(function(data, status, headers, config) {
          $scope.forms = data;
    ...
    // load filter columns names
    $scope.getColumnsNames = function() {
      return $http({
        method: "GET",
        url: "/rest/Filters-config/columnsnames/" + $scope.definition.key
      })
        .success(function(data, status, headers, config) {
          // Filter config found for process definitions
          $scope.filterConfigLoaded = true;
          data
            .filter(v => v.key != "id")
            .forEach(val => {
              $scope.columnsNamesAndType.push(val);
            });
        })
        .error(function(data, status, headers, config) {
          $scope.addAlert(
            $translate.instant("FILTERS-CONFIG.ALERT.NO-FILTER-CONFIG"),
            "warning"
          );
        });
    };
    ...
    $scope.executeWhenReady(function() {
      ...
      $http({
        method: "GET",
        url: "/rest/activiti/process-definitions/" + $routeParams.definitionId
      })
        .success(function(data, status, headers, config) {
          $scope.definition = data;
          $scope.loadProcessInstances();
          ...
          $scope.tabData.tabs.push({
            id: "decisionTables",
            name: "PROCESS-DEFINITION.TITLE.DECISION-TABLES"
          });
          $scope.tabData.tabs.push({
            id: "forms",
            name: "PROCESS-DEFINITION.TITLE.FORMS"
          });
        ...
        .error(function(data, status, headers, config) {
          if (data && data.message) {
            ...
            $rootScope.addAlert(
              $translate.instant("ALERT.GENERAL.HTTP-ERROR"),
              "error"
            );
          }
        });
      ...
      $q.all([
        $translate("PROCESS-INSTANCES.HEADER.ID"),
        ...
        var stateTemplate =
          '<div><div class="ngCellText">{{row.getProperty("endTime") && "Completed" || "Active"}}</div></div>';
        var dateTemplate =
          '<div><div class="ngCellText" title="{{row.getProperty(col.field) | dateformat:\'full\'}}">{{row.getProperty(col.field) | dateformat}}</div></div>';
        ...
        $scope.gridProcessInstances = {
          ...
          enableSorting: false,
          rowHeight: 36,
          selectedItems: $scope.selectedInstances,
      ...
      $q.all([
        $translate("DECISION-TABLES.HEADER.ID"),
...
activitiAdminApp.controller("EditProcessDefinitionCategoryModalCrtl", [
  ...
  "$http",
  "definition",
  function($rootScope, $scope, $modalInstance, $http, definition) {
    $scope.model = {
      id: definition.id,
    ...
    $scope.ok = function() {
      ...
      $http({
        method: "PUT",
        url: "/rest/activiti/process-definitions/" + $scope.model.id,
        data: data
      })
        .success(function(data, status, headers, config) {
...
activitiAdminApp.controller("ShowProcessDefinitionDiagramPopupCrtl", [
  ...
  "definition",
  "$timeout",
  function($rootScope, $scope, $modalInstance, $http, definition, $timeout) {
    $scope.model = {
      id: definition.id,
      name: definition.name
    };
    ...
    $timeout(function() {
      ...
      $("#bpmnModel").load(
        "./display/displaymodel.html?definitionId=" + definition.id
      );
    }, 200);
  }
]);
