    ...
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q) {
		...
		$scope.deleteDeployment = function() {
            ...
            modalInstance.result.then(function (result) {
                if (result === true) {
                    $http({method: 'DELETE', url: '/rest/activiti/deployments/' + $routeParams.deploymentId}).
		...
		$scope.executeWhenReady(function() {
		    // Load deployment
		    $http({method: 'GET', url: '/rest/activiti/deployments/' + $routeParams.deploymentId}).
  	    	    success(function(data, status, headers, config) {
  	    	        $scope.deployment = data;

                    $http({method: 'GET', url: '/rest/activiti/app?deploymentId=' + $routeParams.deploymentId}).
		    ...
		    $http({method: 'GET', url: '/rest/activiti/process-definitions?deploymentId=' + $routeParams.deploymentId}).
  	    	    success(function(data, status, headers, config) {
