...
activitiAdminApp.controller('ProcessInstanceController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q, gridConstants) {
		...
		$scope.showProcessDiagram = function() {
		     $timeout(function() {
          $("#bpmnModel").attr("data-instance-id", $scope.process.id);
          $("#bpmnModel").attr("data-definition-id", $scope.process.processDefinitionId);
          $("#bpmnModel").attr("data-server-id", $rootScope.activeServer.id);
          $("#bpmnModel").load("./display/displaymodel.html?instanceId=" + $scope.process.id);
        }, 200);
		};
		...
		$scope.loadProcessDefinition = function() {
		    // Load definition
		    $http({method: 'GET', url: '/rest/activiti/process-definitions/' + $scope.process.processDefinitionId}).
		...
		$scope.loadProcessInstance = function() {
			...
			// Load process
			$http({method: 'GET', url: '/rest/activiti/process-instances/' + $routeParams.processInstanceId}).
			success(function(data, status, headers, config) {
				$scope.process = data;
        ...
        $scope.tabData.tabs.push({id: 'functionalEntities', name: 'PROCESS-INSTANCE.TITLE.FUNCTIONAL-ENTITIES'});
        $scope.loadDecisionTables();
        $scope.loadForms();
        $scope.showProcessDiagram();
   ...
   $q.all([$translate('FUNCTIONAL-ENTITIES.HEADER.NAME'),
            $translate('FUNCTIONAL-ENTITIES.HEADER.VALUE'),
            $translate('FUNCTIONAL-ENTITIES.HEADER.TYPE')])
   .then(function(headers) {
        var keyTemplate = '<div><div class="ngCellText"><b>{{row.getProperty(col.field) && row.getProperty(col.field) || "(None)"}}</b></div></div>';
        var valueTemplate = '<div>\
                                <div class="ngCellText" ng-if="row.getProperty(\'type\') != \'date\'">{{row.getProperty(col.field) && row.getProperty(col.field) || "(None)"}}</div>\
                                <div class="ngCellText" ng-if="row.getProperty(\'type\') == \'date\'">{{row.getProperty(col.field) && row.getProperty(col.field) || "(None)" | dateformat}}</div>\
                             </div>';
        // Config for variable grid
        $scope.gridFunctionalEntities = {
            data: 'functionalEntities',
            enableRowReordering: false,
            multiSelect: false,
            keepLastSelected : false,
            enableSorting: false,
            rowHeight: 36,
            columnDefs: [
                { field: 'key', displayName: headers[0],cellTemplate: keyTemplate},
                { field: 'value', displayName: headers[1], cellTemplate: valueTemplate},
    ...
    .then(function(headers) {
	  ...
	  $q.all([$translate('SUBMITTED-FORM.HEADER.ID'),
		...
		$scope.loadTasks = function() {
			...
			$http({method: 'GET', url: '/rest/activiti/process-instances/' + $scope.process.id +'/tasks'}).
			success(function(data, status, headers, config) {
				$scope.tasks = data;
		...
		$scope.loadVariables = function() {
			...
			$http({method: 'GET', url: '/rest/activiti/process-instances/' + $scope.process.id +'/variables'}).
			    success(function(data, status, headers, config) {
		...
		$scope.loadSubProcesses = function() {
			...
			$http({method: 'GET', url: '/rest/activiti/process-instances/' + $scope.process.id +'/subprocesses'}).
			success(function(data, status, headers, config) {
		...
		$scope.loadJobs = function() {
			...
			$http({method: 'GET', url: '/rest/activiti/process-instances/' + $scope.process.id +'/jobs'}).
			success(function(data, status, headers, config) {
				$scope.jobs = data;
        ...
        $scope.loadDecisionTables = function() {
            ...
            $http({method: 'GET', url: '/rest/activiti/process-instances/' + $scope.process.id +'/decision-tasks'}).
            success(function(data, status, headers, config) {
        ...
        $scope.loadForms = function() {
            // Load forms
            $http({method: 'GET', url: '/rest/activiti/process-submitted-forms/' + $scope.process.id}).
            success(function(data, status, headers, config) {
                $scope.forms = data;
        ...
        $scope.loadFunctionalEntities = function() {
            // Load forms
            $http({method: 'GET', url: '/rest/activiti/process-instances/' +  $scope.definition.key + '/' + $scope.process.id +'/functionalEntity'}).
            success(function(data, status, headers, config) {
                $scope.functionalEntities = data;
                //$scope.tabData.tabs[6].info = data.total;
            }).
            error(function(data, status, headers, config) {
            });
        };
...
activitiAdminApp.controller('DeleteProcessModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  ...
	  $http({method: 'POST', url: '/rest/activiti/process-instances/' + $scope.process.id,
		  data: dataForPost
      }).
        success(function(data, status, headers, config) {
...
activitiAdminApp.controller('UpdateVariableCrtl',
    ...
    $scope.executeUpdateVariable = function() {
        ...
        $http({method: 'PUT', url: '/rest/activiti/process-instances/' + processInstanceId + '/variables/' + $scope.updateVariable.name, data: dataForPut}).
            success(function(data, status, headers, config) {
                $modalInstance.close(true);
                $scope.status.loading = false;
            }).
...
activitiAdminApp.controller('DeleteVariableCrtl',
    ...
    $scope.deleteVariable = function() {
         $http({method: 'DELETE', url: '/rest/activiti/process-instances/' + processInstanceId + '/variables/' + $scope.variable.name}).
            success(function(data, status, headers, config) {
                $modalInstance.close(true);
                $scope.status.loading = false;
            }).
...
activitiAdminApp.controller('AddVariableCrtl',
    ...
    $scope.createVariable = function() {
        ...
        var data = {
         ...
         }else if ($scope.newVariable.type === 'ArrayList') {

            data.value = $scope.newVariable.value.split(',');

         } 

         $http({method: 'POST', url: '/rest/activiti/process-instances/' + processInstanceId + '/variables', data:data}).
            success(function(data, status, headers, config) {
                $modalInstance.close(true);
                $scope.status.loading = false;
            }).
