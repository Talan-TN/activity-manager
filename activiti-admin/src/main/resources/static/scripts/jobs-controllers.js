...
activitiAdminApp.controller("JobsController", [
  "$scope",
  "$rootScope",
  "$http",
  "$timeout",
  ...
  function(
    ...
    if ($rootScope.filters.forced.jobFilter) {
      ...
      for (var prop in $rootScope.filters.forced.jobFilter) {
        $scope.filter.addProperty(
          { id: prop },
          $rootScope.filters.forced.jobFilter[prop]
        );
      }
    ...
    $scope.processQueryResponse = function(jobsResponse) {
      for (var i = 0; i < jobsResponse.data.length; i++) {
        jobsResponse.data[
          i
        ].processDefinition = $rootScope.getProcessDefinitionFromCache(
          jobsResponse.data[i].processDefinitionId
        );
      }
    ...
    $scope.processDefinitionFilterChanged = function() {
      if (
        $scope.filter.processDefinition &&
        $scope.filter.processDefinition !== "-1"
      ) {
        $scope.filter.properties.processDefinitionId =
          $scope.filter.processDefinition;
        $scope.filter.refresh();
      } else {
        var tempProcessDefinitionId =
          $scope.filter.properties.processDefinitionId;
