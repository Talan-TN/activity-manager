  ...
  .controller('AppDefinitionCtrl', ['$rootScope', '$scope', '$translate', '$http', '$location', '$routeParams', '$modal', '$popover', '$timeout',
    ...
    $scope.loadApp = function() {
    	...
    	if ($routeParams.modelHistoryId) {
    		...
    		definitionUrl = ACTIVITI.CONFIG.contextRoot + '/rest/app-definitions/' + $routeParams.modelId
                + '/history/' + $routeParams.modelHistoryId;
    	} else {
    		url = ACTIVITI.CONFIG.contextRoot + '/rest/models/' + $routeParams.modelId;
    		definitionUrl = ACTIVITI.CONFIG.contextRoot + '/rest/app-definitions/' + $routeParams.modelId;

    		$scope.model.appExportUrl = ACTIVITI.CONFIG.contextRoot + '/rest/app-definitions/' + $routeParams.modelId +
...
.controller('PublishAppDefinitionPopupCtrl', ['$rootScope', '$scope', '$http', '$route', '$translate', function ($rootScope, $scope, $http, $route, $translate) {
    ...
    $scope.ok = function (force) {
        ...
        $http({method: 'POST', url: ACTIVITI.CONFIG.contextRoot + '/rest/app-definitions/' + $scope.model.app.id + '/publish', data: data}).
            success(function(data, status, headers, config) {
                $scope.$hide();
...
.controller('ImportNewVersionAppDefinitionCtrl', ['$rootScope', '$scope', '$http', 'Upload', '$route', function ($rootScope, $scope, $http, Upload, $route) {
  ...
  $scope.onFileSelect = function($files, isIE) {
      ...
      for (var i = 0; i < $files.length; i++) {
          ...
          var url;
          if (isIE) {
              url = ACTIVITI.CONFIG.contextRoot + '/rest/app-definitions/' + $scope.model.app.id + '/text/import?renewIdmEntries=' + $scope.popup.renewIdmIds;
          } else {
              url = ACTIVITI.CONFIG.contextRoot + '/rest/app-definitions/' + $scope.model.app.id + '/import?renewIdmEntries=' + $scope.popup.renewIdmIds;
          }
