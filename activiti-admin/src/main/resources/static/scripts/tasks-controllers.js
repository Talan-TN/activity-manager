...
activitiAdminApp.controller("TasksController", [
  "$scope",
  "$rootScope",
  "$http",
  "$timeout",
  ...
  function(
    ...
    $scope.variableFilterOperators =
      ActivitiAdmin.Utils.variableFilterOperators;

    var filterConfig = {
      url: "/rest/activiti/tasks",
      method: "POST",
    ...
    if ($rootScope.filters.forced.taskFilter) {
      ...
      $scope.filter = new ActivitiAdmin.Utils.Filter(
        filterConfig,
        $http,
        $timeout,
        $rootScope
      ...
      for (var prop in $rootScope.filters.forced.taskFilter) {
        $scope.filter.addProperty(
          { id: prop },
          $rootScope.filters.forced.taskFilter[prop]
        );
      }
      ...
      if (prop == "processVariables" || prop == "taskVariables") {
        ...
        for (var i = 0; i < value.length; i++) {
          ...
          if (
            ...
            if (
              variable.type.id == "long" ||
              variable.type.id == "short" ||
              variable.type.id == "double" ||
              variable.type.id == "integer"
            ) {
    ...
    $scope.highlightVariableValue = function(varFilter) {
      ...
      for (
        var i = 0;
        i < $scope.filter.properties[varFilter.scope].length;
        i++
      ) {
        ...
        $timeout(function() {
          var formField = $(
            "#filter-" +
              prop.id +
              "-name-" +
              ($scope.filter.properties[prop.id].length - 1)
          );
          formField.focus();
        }, 100);
      }
    ...
    $q.all([
      ...
      $scope.gridDefinitions = {
        ...
        columnDefs: [
          ...
          {
            field: "endTime",
            displayName: headers[5],
            cellTemplate: gridConstants.dateTemplate
          },
          {
            field: "priority",
            displayName: headers[6],
            cellTemplate: gridConstants.defaultTemplate
          }
        ]
      };
    ...
    // load assignees fullNames and ids
    $scope.loadAssignees = function() {
      var promise = $http({
        method: "GET",
        url: "/rest/filter-data/assignnes"
      });

      return promise;
    };

    $scope.loadAssignees().success(function(response) {
      $scope.assignnes = response;
    });

    $scope.tasksFilterChanged = function() {
      console.log("$scope.selectedAssignee", $scope.filter.selectedAssignee);
      if ($scope.filter.selectedAssignee != -1) {
        $scope.filter.properties.taskAssignee = $scope.filter.selectedAssignee;
        $scope.filter.refresh();
      } else {
        var tempTask = $scope.filter.properties.tempTask;
        $scope.filter.properties.taskAssignee = null;
