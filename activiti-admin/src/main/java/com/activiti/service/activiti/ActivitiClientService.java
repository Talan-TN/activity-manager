...
public class ActivitiClientService {
	...
	@Autowired
	protected EncryptingService encryptingService;

	@Autowired
	protected ObjectMapper objectMapper;

    public CloseableHttpClient getHttpClient(ServerConfig serverConfig) {
        return getHttpClient(serverConfig.getUserName(), encryptingService.decrypt(serverConfig.getPassword()));
    }
    ...
    public JsonNode executeRequest(HttpUriRequest request, ServerConfig serverConfig, int expectedStatusCode) {
        return executeRequest(request, serverConfig.getUserName(), encryptingService.decrypt(serverConfig.getPassword()), expectedStatusCode);
    }
	...
	public JsonNode executeDownloadRequest(HttpUriRequest request, HttpServletResponse httpResponse, ServerConfig serverConfig, int expectedStatusCode) {
        return executeDownloadRequest(request, httpResponse, serverConfig.getUserName(), encryptingService.decrypt(serverConfig.getPassword()), expectedStatusCode);
    }
    ...
    public AttachmentResponseInfo executeDownloadRequest(HttpUriRequest request, ServerConfig serverConfig, Integer... expectedStatusCodes) {
        return executeDownloadRequest(request, serverConfig.getUserName(), encryptingService.decrypt(serverConfig.getPassword()), expectedStatusCodes);
    }
    ...
    public ResponseInfo execute(HttpUriRequest request, ServerConfig serverConfig, int... expectedStatusCodes) {
        return execute(request, serverConfig.getUserName(), encryptingService.decrypt(serverConfig.getPassword()), expectedStatusCodes);
    }
    ...
    public void execute(HttpUriRequest request, HttpServletResponse httpResponse, ServerConfig serverConfig) {
        execute(request, httpResponse, serverConfig.getUserName(), encryptingService.decrypt(serverConfig.getPassword()));
    }
			...
			if (!success) {
				...
				try {
					if (response.getEntity() != null && response.getEntity().getContentLength() != 0) {
						InputStream responseContent = response.getEntity().getContent();
						JsonNode errorBody = objectMapper.readTree(responseContent);
						errorMessage = extractError(errorBody,  errorBody.get("exception").asText());
