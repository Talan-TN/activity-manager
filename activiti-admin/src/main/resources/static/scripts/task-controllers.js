...
activitiAdminApp.controller('TaskController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q, gridConstants) {
		...
		$scope.loadTask = function() {
			...
			// Load task
			$http({method: 'GET', url: '/rest/activiti/tasks/' + $routeParams.taskId}).
   ...
   .then(function(headers) {
		...
		$scope.loadSubTasks = function() {
			...
			$http({method: 'GET', url: '/rest/activiti/tasks/' + $scope.task.id +'/subtasks'}).
			success(function(data, status, headers, config) {
				$scope.subTasks = data;
		...
		$scope.loadVariables = function() {
			...
			$http({method: 'GET', url: '/rest/activiti/tasks/' + $scope.task.id +'/variables'}).
			success(function(data, status, headers, config) {
		...
		$scope.loadIdentityLinks = function() {
			...
			$http({method: 'GET', url: '/rest/activiti/tasks/' + $scope.task.id +'/identitylinks'}).
			success(function(data, status, headers, config) {
		...
		$scope.showTaskForm = function() {
		    if($scope.task.endTime) {
		        $http({method: 'GET', url: '/rest/activiti/task-submitted-form/' + $scope.task.id}).
	            success(function(data, status, headers, config) {
      ...
      $scope.loadTask();
		...
		$scope.assignTask = function() {
			...
			modalInstance.result.then(function (user) {
				if(user !== undefined) {
					if(user == '') {
						$scope.addAlert('Task reassigned ', 'info');
...
activitiAdminApp.controller('CompleteModalInstanceCrtl',
    ['$rootScope', '$scope', '$modalInstance','$timeout', '$http', '$translate', 'task', function ($rootScope, $scope, $modalInstance,$timeout, $http, $translate, task) {
  ...
  $scope.ok = function () {
	  ...
	  $http({method: 'POST', url: '/rest/activiti/tasks/' + $scope.task.id, data: {action: 'complete'}}).
  	  success(function(data, status, headers, config) {
    	 ...
    	 $scope.status.loading = false;
    	 $scope.addAlert($translate.instant('ALERT.TASK.ERROR.ERROR-400')+' '+data.message, 'error');
      });
  };
...
activitiAdminApp.controller('ResolveTaskModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'POST', url: '/rest/activiti/tasks/' + $scope.task.id, data: {action: 'resolve'}}).
  	  success(function(data, status, headers, config) {
  		$modalInstance.close(true);
  		$scope.status.loading = false;
      }).error(function(data, status, headers, config) {
...
activitiAdminApp.controller('DeleteTaskModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'DELETE', url: '/rest/activiti/tasks/' + $scope.task.id}).
    	success(function(data, status, headers, config) {
    		$modalInstance.close(true);
	  		$scope.status.loading = false;
        }).
...
activitiAdminApp.controller('DelegateModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'POST', url: '/rest/activiti/tasks/' + $scope.task.id, data: {action: 'delegate', assignee: $scope.status.user}}).
  	  success(function(data, status, headers, config) {
...
activitiAdminApp.controller('AssignModalInstanceCrtl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
	  ...
	  $scope.ok = function () {
		  ...
		  $http({method: 'PUT', url: '/rest/activiti/tasks/' + $scope.task.id, data: {assignee: $scope.newAssignee}}).
	  	  success(function(data, status, headers, config) {
    ...
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
	  ...
	  $scope.ok = function () {
		  ...
		  var dataForPut = {
			  ...
			  assignee: $scope.model.assignee,
			  dueDate: task.dueDate,
			  priority: $scope.model.priority,
			  category: $scope.model.category,
		  };

		  $http({method: 'PUT', url: '/rest/activiti/tasks/' + $scope.task.id, data: $scope.model}).
	  	  success(function(data, status, headers, config) {
	  		$modalInstance.close(true);
	  		$scope.status.loading = false;
	      }).error(function(data, status, headers, config) {
