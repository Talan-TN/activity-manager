...
.controller('ImportAppDefinitionCtrl', ['$rootScope', '$scope', '$http', 'Upload', '$location', function ($rootScope, $scope, $http, Upload, $location) {
  ...
  $scope.onFileSelect = function($files, isIE) {
      ...
      for (var i = 0; i < $files.length; i++) {
          ...
          var url;
          if (isIE) {
             url = ACTIVITI.CONFIG.contextRoot + '/rest/app-definitions/text/import?renewIdmEntries=' + $scope.model.renewIdmIds;
          } else {
              url = ACTIVITI.CONFIG.contextRoot + '/rest/app-definitions/import?renewIdmEntries=' + $scope.model.renewIdmIds;
          }
          Upload.upload({
              url: url,
              method: 'POST',
