...
activitiAdminApp.controller('AppDeploymentController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q) {
        ...
        $scope.showAllDecisionTables = function() {
		 ...
		 $scope.deleteApp = function() {
		     ...
		     modalInstance.result.then(function (result) {
		         if (result === true) {
		             $http({method: 'DELETE', url: '/rest/activiti/apps/' + $routeParams.appId}).
		...
		$scope.executeWhenReady(function() {
		    // Load deployment
		    $http({method: 'GET', url: '/rest/activiti/apps/' + $routeParams.appId}).
  	    	    success(function(data, status, headers, config) {
  	    	        $scope.appDeployment = data;
  	    	        $scope.downloadAppUrl = "/rest/activiti/apps/export/" + data.deploymentId;

                    // Load process definitions
                    $http({method: 'GET', url: '/rest/activiti/apps/process-definitions/' + data.deploymentId}).
                    ...
                    if (data.dmnDeploymentId) {
                        $http({method: 'GET', url: '/rest/activiti/apps/decision-tables/' + data.dmnDeploymentId}).
                    ...
                    // Load forms
                    $http({method: 'GET', url: '/rest/activiti/forms', params: {appId:data.id}}).
...
activitiAdminApp.controller('RedpoloyAppCrtl',
    ['$scope', '$modalInstance', '$http', 'app', function ($scope, $modalInstance, $http, app) {
        ...
        $scope.deployApp = function () {
            ...
            $http({method: 'GET',
                url: '/rest/activiti/apps/redeploy/'+app.deploymentId
            }).
            success(function(data, status, headers, config) {
                $scope.status.loading = false;
                if (data.error) {
        ...
        $scope.deployAppAsNewVersion = function (appId) {
            $scope.status.loading = true;
            $http({method: 'GET',
                url: '/rest/activiti/apps/redeploy/' + app.deploymentId + '/' + appId
            }).
            success(function(data, status, headers, config) {
                $scope.status.loading = false;
                if (data.error) {
