...
var activitiAdminApp = angular.module('activitiAdminApp', ['http-auth-interceptor', 'ngResource', 'ngRoute', 'ngCookies',
    ...
    'ui.grid', 'ui.grid.edit', 'ui.grid.selection', 'ui.grid.autoResize', 'ui.grid.moveColumns', 'ui.grid.cellNav','mc.resizer']);
	           ...
	           $rootScope.$on('event:auth-authConfirmed', function() {
	               if (!$rootScope.authenticated) {
	                   ...
	                   $rootScope.account = Account.get(function () {
                           ...
                           $rootScope.loadDatabaseConfig(false);
	                       $rootScope.loadFunctionalMapping(false);
	                       $rootScope.loadFiltersConfig(false);
                           /* end nbensaber*/
	                   }, function () {
	                       $rootScope.authenticated = false;
	                       $location.path('/login');
	            ...
	            $rootScope.$on('event:auth-loginConfirmed', function() {
	                if (!$rootScope.authenticated) {
	                    ...
	                    $rootScope.account = Account.get(function () {
	                        ...
	                        $rootScope.loadDatabaseConfig(true);
	                        $rootScope.loadFunctionalMapping(true);
	                        $rootScope.loadFiltersConfig(true);
	            ...
	            $rootScope.$on('event:auth-loginCancelled', function() {
	                ...
	                $rootScope.DatabaseConfigLoaded = false;
	                $location.path('');
            });
        }])
        .run(['$rootScope', '$http', '$timeout', '$location', '$cookies', '$modal', '$translate',
            function($rootScope, $http, $timeout, $location, $cookies, $modal, $translate) {
                ...
                $rootScope.loadServerConfig = function(callbackAfterLoad) {
                    $http({method: 'GET', url: '/rest/server-configs'}).
                ...
                $rootScope.loadDatabaseConfig = function(callbackAfterLoad) {
                    $http({method: 'GET', url: '/rest/database-configs'}).
                    success(function(data) {
                        if (data) {
                            $rootScope.databaseConfigData = data;
                            $rootScope.DatabaseConfigLoaded = true;
                        } else {
                            console.log('Warning! No database configurations received');
                        }
                    }).
                    error(function(data, status, headers, config) {
                        console.log('Something went wrong: ' + data);
                ...
                $rootScope.functionalEntitiesMappingLoaded = false;

                $rootScope.loadFunctionalMapping = function(callbackAfterLoad) {
                    $http({method: 'GET', url: '/rest/functional-mapping'}).
                    success(function(data) {
                        if (data) {
                            $rootScope.functionalMappingData = data;
                            $rootScope.FunctionalEntitiesMappingLoaded = true;
                        } else {
                            console.log('Warning! No Functional Entities Mapping received');
                        }
                    }).
                    error(function(data, status, headers, config) {
                        console.log('Something went wrong: ' + data);
                ...
                $rootScope.filtersConfigLoaded = false;

                $rootScope.loadFiltersConfig = function(callbackAfterLoad) {
                    $http({method: 'GET', url: '/rest/filters-config'}).
                    success(function(data) {
                        if (data) {
                            $rootScope.filtersConfigData = data;
                            $rootScope.filtersConfigLoaded = true;
                        } else {
                            console.log('Warning! No filters config received');
                        }
                    }).
                    error(function(data, status, headers, config) {
                        console.log('Something went wrong: ' + data);
	        	...
	        	$rootScope.loadProcessDefinitionsCache = function() {
                    if ($rootScope.activeServer && $rootScope.activeServer.id) {
                        var promise = $http({
                            method: 'GET',
                            url: '/rest/activiti/process-definitions?size=100000000'
