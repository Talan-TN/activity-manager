...
activitiAdminApp.controller('DeleteUserModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  ...
	  $http({method: 'DELETE', url: '/rest/users/' + $scope.user.login}).
    	success(function(data, status, headers, config) {
...
activitiAdminApp.controller('EditUserModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'PUT', url: '/rest/users/' + $scope.user.login, data: $scope.model}).
  	  success(function(data, status, headers, config) {
  		  $modalInstance.close(true);
  		  $scope.status.loading = false;
      }).error(function(data, status, headers, config) {
...
activitiAdminApp.controller('ChangePasswordModalInstanceCrtl',
    ['$scope', '$modalInstance', '$http', 'user', function ($scope, $modalInstance, $http, user) {
	  ...
	  $scope.ok = function () {
		  $scope.status.loading = true;
		  $http({method: 'PUT', url: '/rest/users/' + $scope.user.login + '/change-password', data: $scope.model}).
	  	  success(function(data, status, headers, config) {
	  		  $modalInstance.close(true);
	  		  $scope.status.loading = false;
	      }).error(function(data, status, headers, config) {
...
activitiAdminApp.controller('NewUserModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'POST', url: '/rest/users', data: $scope.model, ignoreErrors: true}).
  	  success(function(data, status, headers, config) {
